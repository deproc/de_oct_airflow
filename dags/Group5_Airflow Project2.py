...
SQL_INSERT_TO_FACT = f'INSERT INTO "ETL_AF"."DEV_DB"."FACT_STOCK_HISTORY_GROUP5" SELECT C.ID, S.DATE,S.OPEN, S.HIGH, S.LOW, S.CLOSE, S.VOLUME, S.ADJCLOSE'+
f' FROM "US_STOCKS_DAILY"."PUBLIC"."STOCK_HISTORY" s' +
f'LEFT JOIN "US_STOCKS_DAILY"."PUBLIC"."COMPANY_PROFILE" c ON C.SYMBOL = S.SYMBOL' +
f'WHERE DATE = {DATE_PARAMETER}'
...
with DAG(
        DAG_ID,
        start_date=datetime(2022, 11 , 04),
